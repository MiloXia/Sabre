...
RESTful api的支持已经开发完毕，虽然略显粗浅，但是已经可以使用（与Web一起使用互不影响），对于RESTful api的支持是半配置半映射的形式，映射部分在于资源的定位，比如URL：/rest/v1.0/users/1，会去寻找resource目录下的users.js，load进来，至于调用哪个方法，就看users里面的方法怎么定义了，让我们来看看。<br />
...
```
你已经注意到了和action方法一样的一个rest函数，它会返回一个对象并调用该对象的api方法定义method和path就行了，很简单。<br />
